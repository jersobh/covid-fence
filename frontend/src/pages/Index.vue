  ...
  methods: {
    ...
    connectWS () {
      ...
      if (this.ws === null) {
        ...
        self.ws.onopen = function (event) {
          ...
          self.recenter()
          if (self.isTracking && self.ws.readyState === 1) {
            self.ws.send(JSON.stringify({'id': self.id, 'lat': self.location.latitude, 'lng': self.location.longitude, 'type': 'get_clients'}))
          }
        }
        self.ws.onmessage = function (event) {
          let msg = JSON.parse(event.data)
          // console.log(msg)
          self.heatmap.splice(0, self.heatmap.length)

          if (msg.type === 'near') {
            for (let i = 0; i < msg.clients.length; i++) {
              var mapLocation = L.latLng(msg.clients[i].lat, msg.clients[i].lon)
              self.heatmap.push(mapLocation)
            }
            console.log(msg.clients)
            // var mapLocation = L.latLng(msg.lat, msg.lng)
            // self.heatmap = heat
            if (self.heatmap.length > 5) {
              self.sendPush('AVISO: Você está entrando em uma área de aglomeração!')
            }
          }
        }
      }
    ...
    startTracking () {
      if (navigator.geolocation) {
        ...
        this.watcher = navigator.geolocation.watchPosition(this.registerLocation, null, { maximumAge: 3000, timeout: 10000, enableHighAccuracy: true })
      } else {
    ...
    registerLocation (position) {
      ...
      if (this.location !== this.lastPosition) {
        this.lastPosition = self.location
        ...
        if (self.ws) {
          if (self.ws.readyState === 1) {
            self.ws.send(JSON.stringify(updateLocation))
          }
        }
